...
function parseCsv(d) {
    if(+d.number_of_homeless >= 1) {
        return {
            year: d.year,
            county: d.county,
            state: d.state,
            total_jobs: d.total_jobs,
            total_average_wage: +d.total_average_wage, // Changing name of column/property
            number_of_homeless: +d.number_of_homeless 
        }    
    }
}

d3.csv("./data/2021_wage_job_homeless.csv", parseCsv).then(function(data) {
    ...
    */
    ...
    const margin = {top: 40, left: 60, right: 100, bottom: 300};
    ...
    */
    ...
    const number_of_homeless = {
        min: d3.min(data, function(d) { return d.number_of_homeless; }),
        max: d3.max(data, function(d) { return d.number_of_homeless; })
    };

    const toal_average_wage = {
        min: d3.min(data, function(d) { return d.toal_average_wage; }),
        max: d3.max(data, function(d) { return d.toal_average_wage; })
    };

    const total_jobs = {
        min: d3.min(data, function(d) { return d.total_jobs; }),
        max: d3.max(data, function(d) { return d.total_jobs; })
    };
    ...
    */
    ...
    const county = ["Addison County", "Bennington County", "Caledonia County", "Chittenden County", "Essex County", "Franklin County",
"Grand Isle County", "Lamoille County", "Orange County", "Orleans County", "Rutland County", "Washington County", "Windham County", "Windsor County"];
    ...
    */
    ...
    const xScale = d3.scaleLinear()
        .domain([number_of_homeless.min, number_of_homeless.max])
    ...
    const rScale = d3.scaleSqrt()
        .domain([total_jobs.min, total_jobs.max])
        .range([10, 30]);

    const fillScale = d3.scaleOrdinal()
        .domain(county)
        .range(['#1b9e77','#d95f02','#7570b3', '#A93226', '#F1948A', 
        '#9B59B6', '#2980B9', '#1ABC9C', '#F4D03F', '#D35400', '#979A9A', 
        '#000080', '#FF0000', '#008000', '#FF585D']);
    ...
    */
    const points = svg.selectAll("circle")
        ...
        .enter()
        .append("circle")
            .attr("cx", function(d) { return xScale(d.number_of_homeless); })
            .attr("cy", function(d) { return yScale(d.toal_average_wage); })
            .attr("r", function(d) { return rScale(d.total_jobs)})
            .attr("fill-opacity", 0.2)
            .attr("stroke", function(d) { return fillScale(d.county); })
            .attr("stroke-width", 1.5)
            .attr("fill", function(d) { return fillScale(d.county); })
    ...
    */
    ...
    const yAxisLabel = svg.append("text")
        ...
        .text("Average Wage in thousand USD (US$)");
    ...
    */
    points.on("mouseover", function(e,d) {
        ...
        let displayValue = d3.format(",")(d.number_of_homeless);
        
        tooltip.style("visibility", "visible")
            .style("top", `${y}px`)
            .style("left", `${x}px`)
            .html(`<b>${d.year}</b>
            <br>${displayValue} number_of_homeless`);
